      ...
      <html>
        ...
        <body>
          ...
          <div class="box">
            ...
            <div class="grid"><strong>ESTADO ACTUAL:</strong> <span style="font-weight: bold; color: var(--color-art-caramel);">${trackedOrder.status.toUpperCase()}</span></div>
          </div>
      ...
      {hasSearched && trackedOrder ? (
        <div className="bg-white border border-art-border rounded-lg p-6 shadow-xs space-y-6 animate-fade-in" id="tracking_result_card">
          ...
          <div className="space-y-4">
            <h5 className="text-[12px] uppercase font-bold text-art-brown tracking-wider">Línea del Tiempo de Cocina</h5>
                    ...
                    <div key={idx} className="flex md:flex-col items-center gap-4 md:gap-2 w-full md:w-1/6 text-left md:text-center">
                      ...
                      <div
                        className={`w-8 h-8 rounded-full border-4 flex items-center justify-center font-bold text-xs shrink-0 transition-all duration-300 ${
                          isCompleted
                            ...
                            : isActive
                            ? "bg-white border-art-accent text-art-accent scale-110 shadow-xs animate-pulse"
                            : "bg-white border-art-border text-art-brown"
                        }`}
                      >
                      ...
                      <div className="flex flex-col space-y-0.5">
                        <span className={`text-[12px] font-bold tracking-tight ${
                          isActive
                            ? "text-art-accent font-extrabold text-sm"
                            : isCompleted
                            ? "text-art-text font-bold"
                            : "text-art-brown"
                        }`}>
                          {step}
                        </span>
                        
                        <span className="text-[10px] text-art-brown font-semibold md:hidden block">
          ...
          <div className="border-t border-art-border pt-5 space-y-4">
            <h5 className="text-[12px] uppercase font-bold text-art-brown tracking-wider">Resumen del Pedido</h5>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="bg-art-panel p-4 rounded-xl space-y-2 text-art-text border border-art-border/20">
              ...
              <div className="border border-art-border p-4 rounded-xl bg-white space-y-2.5">
                <p className="font-serif font-bold text-art-text border-b border-art-panel pb-1.5">Productos:</p>
                <div className="space-y-1.5">
                  {trackedOrder.items.map((item, i) => (
                    <div key={i} className="flex justify-between text-art-muted text-[12px] font-medium">
                      <span>{item.name} (x{item.quantity})</span>
                      <span className="font-bold text-art-text">{formatPrice(item.price * item.quantity)}</span>
                    </div>
                  ))}
                </div>
                <div className="pt-2 border-t border-art-panel flex justify-between font-bold text-art-text text-[14px]">
                  <span>Total pagado:</span>
                  <span className="text-art-accent font-extrabold">{formatPrice(trackedOrder.totalAmount)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 bg-art-panel p-4 rounded-xl border border-art-border">
            <button
              onClick={handlePrintBuyerInvoice}
              className="bg-art-text hover:bg-art-brown text-white font-serif font-bold text-xs px-4 py-2.5 rounded-lg transition-all shadow-xs flex items-center gap-2 cursor-pointer border border-art-text"
              id="btn_buyer_print_invoice"
            >
              <Printer className="w-4 h-4 text-art-border" /> Imprimir Mi Comprobante / Factura
            </button>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className={`font-serif font-bold text-xs px-3.5 py-2 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer border ${
                  notificationsEnabled
                    ...
                    : "bg-white border-art-border text-art-text hover:bg-art-panel"
                }`}
              ...
              <a
                ...
                className="bg-wa hover:bg-wa-dark text-white font-bold text-xs px-4 py-2.5 rounded-full transition-all flex items-center gap-1.5 shadow-md"
              >
          ...
          <div className="space-y-3">
            {historyOrders.map((histOrder) => (
              <div 
                ...
                className="bg-art-card border border-art-line p-4 rounded-lg hover:border-art-accent hover:shadow-sm transition-all cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
