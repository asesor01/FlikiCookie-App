...
export default function AIKitchenChat({ onApplyPreset, currentSpec }: AIKitchenChatProps) {
  ...
  useEffect(() => {
    const box = chatEndRef.current?.closest(".overflow-y-auto"); if (box) { box.scrollTop = box.scrollHeight; }
  }, [messages, isLoading]);
  ...
  return (
    <div id="ai_kitchen_chat_card" className="flex flex-col bg-art-panel border border-art-border rounded-xl h-[520px] shadow-xs overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 bg-art-accent text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/80 tracking-wider uppercase font-semibold">Consejero de Alta Repostería</p>
          </div>
        </div>
